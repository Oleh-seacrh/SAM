...
const EnrichmentTab: React.FC = () => {
  ...
  // 1) Завантажити конфіг з бекенду при відкритті
  useEffect(() => {
    let ignore = false;
    (async () => {
      try {
  ...
  // утиліта для оновлення вкладених прапорців
  function toggle<K1 extends keyof EnrichConfig>(key: K1, subkey?: any, subsub?: any) {
    setCfg(prev => {
      const next = structuredClone(prev);
      if (subsub !== undefined) {
        // @ts-ignore
        next[key][subkey][subsub] = !next[key][subkey][subsub];
      } else if (subkey !== undefined) {
        // @ts-ignore
        next[key][subkey] = !next[key][subkey];
      } else {
        // @ts-ignore
        next[key] = !next[key];
      }
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="space-y-2">
          {/* Поки це тільки візуальні чекбокси — підв’язку до схеми зробимо пізніше */}
          <label className="flex items-center gap-2">
          ...
          <label className="flex items-center gap-2">
            <input type="checkbox" />
            <span>Tags (Medical / NDT / Other)</span>
          </label>
        </div>
      </section>
    </div>
