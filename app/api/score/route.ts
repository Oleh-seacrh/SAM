 ...
 * Prompt Build
 * ================================================== */
function buildPrompt(userPrompt: string, enriched: (Item & { content: string })[]) {
  ...
  const jsonShape = `Return ONLY raw JSON (no markdown/backticks) with EXACT shape:
{
  "scoresByDomain": {
    "<domain>": {
      "label": "good" | "maybe" | "bad",
...
Infer EVERYTHING only from provided data (title, snippet, homepage content). ALWAYS include ALL keys for every domain.
Rules:
...
Country: only if explicitly present (textual name, address, phone code + context). Otherwise null.
CompanyType: manufacturer / distributor / dealer / other (if unclear -> other).
Brands: ONLY tokens explicitly in text; dedupe; do not invent.
Confidence: 0..1 float.
If unknown: use null or [] (never omit keys).
...
function normalizeScores(resp: any): LLMResponse {
  ...
  for (const [rawDomain, rawScore] of Object.entries<any>(core)) {
    ...
    const domain = stripWWW(rawDomain.toLowerCase());
    ...
    let countryISO2: string | null =
      rawScore.countryISO2 ||
      rawScore.country_iso2 ||
      rawScore.country ||
      null;
      ...
      const r = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          temperature: 0,
            ...
            {
              role: "system",
              content: "You are a precise extraction assistant. Output ONLY raw JSON.",
            },
