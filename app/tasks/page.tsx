...
type BoardPayload = {
  board: { id: number; name: string; columns: Column[]; tasks: Task[] };
};
...
export default function TasksPage() {
  ...
  const [columnKey, setColumnKey] =
    useState<"todo" | "inprogress" | "done" | "blocked">("todo");
  ...
  async function saveDueDate(dateStr: string) {
    if (!detail?.task?.id) return;
    const payload: any = {
      dueAt: dateStr ? new Date(`${dateStr}T00:00:00`).toISOString() : null,
    };
    await fetch(`/api/kanban/tasks/${detail.task.id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        {/* Кольорові бейджі тегів (до 3 штук) */}
          ...
          <div className="space-y-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-[var(--muted)]">
                  Priority: <b>{detail.task.priority}</b> • Status:{" "}
                  <b>{detail.task.status}</b>
                </div>
              </div>
                ...
                <div className="text-sm">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {detail.task.tags.map((t) => (
                      <span
                        key={t}
                        className="text-xs rounded-full px-2 py-0.5 border border-white/10 bg-white/10"
                      >
                        {t}
                      </span>
                    ))}
              ...
              <div className="space-y-2 max-h-64 overflow-auto pr-1">
                {detail.comments.length ? (
                  detail.comments.map((c) => (
                    <div
                      key={c.id}
                      className="rounded-lg border border-white/10 p-2 bg-black/20"
                    >
