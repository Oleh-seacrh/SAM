  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/orgs/dedupe?${params.toString()}`, { method: "GET" });
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/orgs", {
        ...
        body: JSON.stringify({
          name: name || undefined,          // тепер опційне
          domain: domain || undefined,
          emails: emails,
          org_type: defaultType,            // вкладка за змовчанням
        }),
      });
      ...
      if (res.status === 409) {
        ...
        if (data?.error === "DUPLICATE_SOFTLOCK") {
          setDupes(data?.duplicates ?? []);
          setSoftLockOpen(true);            // показуємо гарне діалог-вікно
          setOverride(false);
          return;
        }
        if (data?.error === "DUPLICATE_HARDLOCK") {
          alert(data?.detail || "Duplicate constraint"); // можна зробити красивий toast
          return;
        }
      }
      ...
      onClose();
      reset();
      // TODO: рефреш списку (router.refresh() або invalidate query)
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[50]">
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <form onSubmit={onSubmit}
          ...
          {/* Невеликий прев’ю дублікатів під формою (необов'язково) */}
          {dupes.length > 0 && (
